...
//   console.log(pathname);
  return (
    <div className="hidden md:inline-flex w-1/3 items-center justify-center gap-7 capitalize text-sm font-semibold text-lightColor">
      {headerData?.map((item) => (
        <Link
          key={item?.title}
          href={item.href}
          ...
          // pathname === item?.href ? "text-shop_light_green" : ""  [ternary operator]
          //}`}
        >
          {item.title}
          <span
